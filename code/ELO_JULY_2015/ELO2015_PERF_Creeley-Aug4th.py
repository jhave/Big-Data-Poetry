...
###################################
#
# NLTK POS type check & replace
#
###################################
def findSamePOS(rpw):
    #print "enter findSamePOS, rpw=", rpw

    word_punct_nopunct = import_utilities.strip_punctuation_bool(rpw)
    word_nopunct = word_punct_nopunct['word'].strip(" \n\t\r")
    word_punct = word_punct_nopunct['punct']
    punct_bool = word_punct_nopunct['punct_bool']

    if any(word_nopunct in s for s in RB):
        #print word_nopunct, "is in RB"
        return random.choice(RB)

    if any(word_nopunct in s for s in JJ):
        #print word_nopunct, "is in JJ"
        return random.choice(JJ)

    if any(word_nopunct in s for s in VBD):
        #print word_nopunct, "is in VBD"
        return random.choice(VBD)

    if any(word_nopunct in s for s in VBZ):
        #print word_nopunct, "is in VBZ"
        return random.choice(VBZ)
