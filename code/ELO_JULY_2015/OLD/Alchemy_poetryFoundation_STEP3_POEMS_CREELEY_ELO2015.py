...
#print JJ
print "\n\nTechinical process: the following poems were produced using a 10,000+ corpus sent to Alchemy to produce entity-recognition, POS, and sentiment reports for each poem in JSON. Then using that info to run replacement algorithms using a combination of NLTK synsets and Pattern.en with the average word-length constrained so output reads likeRobert Creeley becoming Samuel Beckett in Gertrude Stein's gut. \n\nLanguage: Python \n\nBlogged here: http://bdp.glia.ca/smaller-words-shrink-gapped"
